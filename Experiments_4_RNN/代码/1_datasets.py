...
class KrakowDataset:
    def __init__(self, sensor=171, is_resample=True):
        """
        输入：sensor：探测器；is_resample：是否重采样
        csv文件包含多个探测器获取的数据
        """
        ...
        raw_data = pd.concat([pd.read_csv(f'/data2/CaoHaikun/DevelopFiles/DeepLearning Course Task/Experiments_4/dataset/Krakow-airquality/{month}.csv') for month in self.month])
...
class TrafficDataset:
    def __init__(self, sensor=10, target=0):
        # 选取10号检测器用作序列数据，预测目标选取第一列车流量
        self.raw_data = np.load('/data2/CaoHaikun/DevelopFiles/DeepLearning Course Task/Experiments_4/dataset/traffic-flow/traffic.npz')['data']
        self.sensor = sensor
        self.target = target
    ...
    #     # 由于不同列之间纲量相差比较大，因此需要按列进行归一化
    #     self.data = self.raw_data
    #     for i in range(self.raw_data.shape[-1]):
    #         max_value = self.raw_data[:, self.sensor, i].max()
    #         min_value = self.raw_data[:, self.sensor, i].min()
    #         self.data[:, self.sensor, i] = (self.raw_data[:, self.sensor, i] - min_value) / (max_value - min_value)

    # # 逆归一化(只需要对目标列进行逆标准化)
    # def denormalize(self, x):
    #     return x * (self.data[:, self.sensor, self.target].max() - self.data[:, self.sensor, self.target].min()) + self.data[:, self.sensor, self.target].min()

    # 构造训练、验证、测试集
    def construct_set(self, train_por=0.6, test_por=0.2, window_size=12, target=0):
        train_x = []
        train_y = []
        val_x = []
        val_y = []
        ...
        train_seqs = self.data[:len_train]
        for i in range(train_seqs.shape[0] - window_size):
            train_x.append(train_seqs[i:i+window_size, self.sensor, :])
            train_y.append(train_seqs[i:i+window_size, self.sensor, target])

        # 验证集
        len_val = int(self.data.shape[0] * (train_por + test_por))
        val_seqs = self.data[len_train:len_val]
        for i in range(val_seqs.shape[0] - window_size):
            val_x.append(val_seqs[i:i+window_size, self.sensor, :])
            val_y.append(val_seqs[i:i+window_size, self.sensor, target])

        # 测试集
        test_seqs = self.data[len_val:]
        for i in range(test_seqs.shape[0] - window_size):
            test_x.append(test_seqs[i:i+window_size, self.sensor, :])
            test_y.append(test_seqs[i:i+window_size, self.sensor, target])
